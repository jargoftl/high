...
import SecondModal from "./SecondModal"; // adjust path if needed
...
const ConnectPage = () => {
  ...
  useEffect(() => {
    const fetchName = async () => {
      try {
        const response = await fetch("https://electric-eel.onrender.com/info");
    ...
    >
      ...
      {showScrollTop && (
        <motion.button
          ...
          className="fixed bottom-6 right-6 bg-gradient-to-r from-emerald-500 to-emerald-900 text-white p-3 rounded-full shadow-md hover:bg-pink-700"
        >
          <FaArrowUp />
        </motion.button>
      )}
